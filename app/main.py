...
@app.route("/")
def index():
    ra = dao.read_products(latest=True)
    print(ra)
    return render_template("index.html", latest_products=ra)
    ...
    if request.method == "POST":
        ...
        if password.strip() != confirm.strip():
            ...
            if dao.add_user(name=name, username=username,
                            password=password, avatar='TAM THOI XOA DI'):
